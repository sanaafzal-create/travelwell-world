            ...
            <div className="safety-card__body">
              ...
              {s.inZone && (
                ...
                <div className="safety-row safety-row--carve">
                  <span className="safety-row__ic"><Icon name="pin" small /></span>
                  <span>
                    {/* Posture-worded when the zone is FCDO-transcribed (David's
                        ruling, 2026-08-21: postures, never the number). A zone
                        without a posture predates the verbatim re-read and keeps
                        the number until its country is re-read. */}
                    <span className="safety-row__k">Why this level:</span> this destination sits in {s.inZone.name}, {s.inZone.posture ? `where the ${ZONE_POSTURE_TEXT[s.inZone.posture]}` : `which the advisory carries at Level ${s.inZone.lvl}`}.{s.inZone.note ? ` ${s.inZone.note}` : ""}

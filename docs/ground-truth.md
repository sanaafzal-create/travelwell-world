...
| Country name→ISO entries (`COUNTRY_ISO`) | 40 | src/data/safety-data.ts:213 · `export const COUNTRY_ISO: Record<string, strin` |
| …of those, countries WITH an advisory row (`safety.json`) | 38 | src/data/safety-data.ts:240 · `export const SAFETY_DATA = safetyJson as Recor` |

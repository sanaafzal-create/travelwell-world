...
- **Connectivity (inventory pipes).** **Duffel first** (flights — 300+ airlines under Duffel's credentials, no IATAN; **Duffel Payments = merchant of record**, so we never hold the card; sandbox free to wire now). Then **hotel aggregators** (Hotelbeds / TBO / Expedia for volume, consortia for the ultra crown — supplier/aggregator is MoR). Then the **villa ladder** (Exceptional Villas, Top Villas, Nocturne/WIMCO ultra, Luxury Experiences TCI — commission/advisor, no deposit). **CLIA cruise/dive/expedition** is bookable now (we hold CLIA).
